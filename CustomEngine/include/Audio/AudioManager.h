...
namespace Frac
{
	...
	public:
		...
		/**
		 * Should be called when closing the engine to make sure everything gets properly deleted.
		 */
		void ShutDown();
		
		/**
		 * Update function for the audio manager should be called every frame to update the playback state of the audio
		 * @param deltaTime the time since last frame
		 */
		void Update(float deltaTime) override;
		
		/**
		 * Load an audio bank from file.
		...
		/**
		 * Add an audio event to a component
		...
		/**
		 * Checks whether the component has a specific event
		...
		/**
		 * Play an audio event from a component
		...
		/**
		* Gets the master volume for the entire engine
